...
public class CompteServiceImpl extends CompteServiceGrpc.CompteServiceImplBase {
    ...
    public void findByType(FindByTypeRequest request, StreamObserver<FindByTypeResponse> responseObserver) {
        TypeCompte type = request.getType(); // Assuming type is an enum
        FindByTypeResponse.Builder responseBuilder = FindByTypeResponse.newBuilder();

        for (Compte compte : compteDB.values()) {
            if (compte.getType() == type) { // Enum comparison
    ...
    public void deleteById(DeleteByIdRequest request, StreamObserver<DeleteByIdResponse> responseObserver) {
        ...
        Compte deletedCompte = compteDB.remove(id);

        if (deletedCompte != null) {
            responseObserver.onNext(DeleteByIdResponse.newBuilder().setCompte(deletedCompte).build());
        } else {
            responseObserver.onNext(DeleteByIdResponse.newBuilder().setCompte(deletedCompte).build());
        }
        responseObserver.onCompleted();
